...
export class VerMiPerfil {
  ...
  constructor(private cdr: ChangeDetectorRef) {
    ...
    this.perfilFormProveedor = this.fb.group({
        ...
        next: (p) => { this.proveedor = p;
          if (p.role?.name === 'ROLE_PROVEEDOR') {
            ...
            this.perfilFormProveedor.patchValue({
              organizacion: p.nombreorganizacion,
              direccion: p.direccion,
              email: p.email,
              telefono: p.numerocontacto,
              contrasena: '',
              confirmarContrasena: '',
...
/* Funcionalidades del ver perfil */
  ...
  confirmarEliminarCuenta(event: MouseEvent) {
    ...
    if (this.usuario.role?.name === 'ROLE_ANFITRION') {
      this.anfitrionService.eliminar(this.anfitrion.id!).subscribe({
        next: () => {
          console.log('Cuenta de anfitrión eliminada.');
          this.router.navigate(['']);
        },
        error: (err) => console.error('Error al eliminar anfitrión:', err)
      });
    } else if (this.usuario.role?.name === 'ROLE_PROVEEDOR') {
      this.proveedorService.eliminar(this.proveedor.id!).subscribe({
        next: () => {
          console.log('Cuenta de proveedor eliminada.');
          this.router.navigate(['']);
        },
  ...
  guardarCambios() {
    if (this.usuario.role?.name === 'ROLE_ANFITRION') {
      const actualizado: Anfitrion = {
        ...
        nombre: this.perfilForm.value.nombres?.split(' ')[0] || this.anfitrion.nombre,
        apellido: this.perfilForm.value.nombres?.split(' ').slice(1).join(' ') || this.anfitrion.apellido,
        descripcion: this.perfilForm.value.descripcion,
        foto: this.selectedImagePreview
          ? (this.selectedImagePreview as string).split(',')[1]
          : (
            this.anfitrion.foto?.startsWith('data:image')
              ? this.anfitrion.foto.split(',')[1]
              : this.anfitrion.foto || ''
          )
      };
      this.anfitrionService.actualizar(actualizado).subscribe({
        next: () => {
          alert('Perfil actualizado correctamente.');
          this.anfitrion = {
            ...actualizado,
            foto: actualizado.foto
              ? `data:image/jpeg;base64,${actualizado.foto}`
              : this.anfitrion.foto
          };
          this.currentUserData = this.anfitrion;
          this.modoEdicion = false;
          this.selectedImagePreview = null;
        },
        error: (err) => {
          console.error('Error al actualizar anfitrión:', err);
          alert('Error al actualizar el perfil.');
        }
      });
    }
    if (this.usuario.role?.name === 'ROLE_PROVEEDOR') {
      const actualizado: Proveedor = {
        ...this.proveedor,
        ...this.perfilFormProveedor.value,
        id: this.proveedor.id,
        descripcion: this.perfilFormProveedor.value.descripcion,
        foto: this.selectedImagePreview
          ? (this.selectedImagePreview as string).split(',')[1]
          : (
            this.proveedor.foto?.startsWith('data:image')
              ? this.proveedor.foto.split(',')[1]
              : this.proveedor.foto || ''
          )
      };
      this.proveedorService.actualizar(actualizado).subscribe({
        next: () => {
          alert('Perfil actualizado correctamente.');
          this.proveedor = {
            ...actualizado,
          ...
          this.currentUserData = this.proveedor;
          this.modoEdicion = false;
          this.selectedImagePreview = null;
        },
        error: (err) => {
